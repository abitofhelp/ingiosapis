...
func (this *Project) EqualVT(that *Project) bool {
	...
	if this.AccessTier != that.AccessTier {
		return false
	}
...
func (m *Project) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.AccessTier != 0 {
		i = encodeVarint(dAtA, i, uint64(m.AccessTier))
		i--
		dAtA[i] = 0x10
	}
	if len(m.Name) > 0 {
...
func (m *Project) MarshalVTStrict() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *Project) MarshalToVTStrict(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVTStrict(dAtA[:size])
}

func (m *Project) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
...
func (m *Project) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field AccessTier", wireType)
			}
			m.AccessTier = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.AccessTier |= AccessTier(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
